...
from sqlalchemy import Column, Date, DateTime, Index, Numeric, String, func

from .base import Base, TimestampMixin


class TechnicianHours(TimestampMixin, Base):
    ...
    technician_proficiency = Column(Numeric(5, 2), nullable=True)
    technician_productivity = Column(Numeric(5, 2), nullable=True)
    period_start = Column(Date, nullable=False)
    period_end = Column(Date, nullable=False)
    imported_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)

    __table_args__ = (Index("ix_technician_hours_name_period", "technician_name", "period_start"),)
